...
We publish our own composition rather than leave it to be discovered (June 11
snapshot, 384 unique mainnet txs): ~25% DeFi strategy execution (Mento swaps,
Aave supply/withdraw), ~26% capital lifecycle (agent funding, cull unwinds,
treasury ops), ~21% reputation attestations, ~16% identity and lineage
(ERC-8004 registrations, spawn/recall provenance), ~8% decision-proof commits,
~4% x402 settlements. Strategy execution is a minority of volume **by
design** — most of what an agent economy does onchain is be accountable:
register, attest, prove, settle. Every category exists because the protocol
requires it, not to inflate a counter, and each entry's rationale says which.
...
"Recomputable" is not a claim we make once — the orchestrator re-derives every
score it just posted and checks it against the actual onchain `giveFeedback`
calldata **after every settle**, publishing the result as
[`docs/epochs/epoch-N-verification.json`](docs/epochs/) (per-agent recomputed
fitness/score, the keccak payload-hash binding, and the reputation tx to check
it against). To reproduce it yourself:

```bash
cd agent && npm run verify:reputation -- <epoch>
```

Epochs 1–7 verify 61/61 (every feedback ever posted), including the epochs
covered by the correction notes above — the published inputs reproduce exactly
what was scored, which is the point.

On the "orchestrator scores its own agents" question: the canonical Reputation
Registry forbids feedback from an identity's own owner — agents self-own their
ERC-8004 identities precisely so the orchestrator (a distinct registered
identity, #9240) can attest to them. The author is disclosed in every feedback
event, the score is locked to the published formula, and the verification
artifact proves it epoch by epoch. This is performance attestation as ERC-8004
intends it, not anonymous third-party praise — and unlike praise, every number
can be checked.
