...
contract SpawnENSRegistry {
    ...
    function registerSubdomain(string calldata label, address addr) external onlyOwner returns (bytes32 node) {
        ...
        _records[node] =
            NameRecord({owner: msg.sender, resolvedAddress: addr, name: fullName, registeredAt: block.timestamp});
    ...
    function getRecord(string calldata label)
        external
        view
        returns (address recordOwner, address resolvedAddress, string memory name, uint256 registeredAt)
    {
        bytes32 node = _computeNode(label);
        NameRecord storage record = _records[node];
