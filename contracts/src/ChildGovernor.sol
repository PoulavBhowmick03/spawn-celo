...
contract ChildGovernor is Initializable {
    ...
    function initialize(address _parent, address _factory, address _governance, uint256 _maxGas) external initializer {
        parent = _parent;
        factory = _factory;
    ...
    function castVote(uint256 proposalId, uint8 support, bytes calldata encryptedRationale)
        external
        onlyAuthorized
        onlyActive
    {
    ...
    function revealRationale(uint256 proposalId, bytes calldata decryptedRationale) external onlyAuthorized {
