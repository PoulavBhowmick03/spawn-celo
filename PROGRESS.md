...
## Sponsor-an-agent: open external capital (2026-06-13)

- **Done**: The swarm is no longer a closed $50 loop — anyone can sponsor an agent. `/sponsor` on the dashboard: connect wallet → send cUSD to the treasury → the orchestrator detects the deposit onchain and spawns a real ERC-8004 agent in the sponsor's name at the next epoch (seeded from the top performer, funded ≤ deposit and ≤ $5, registered + onchain provenance like any agent). Disclosed in UI + README as a sponsorship of an autonomous agent (non-withdrawable); the developer's at-risk capital stays exactly $50 (per-agent/$5 + per-tx/$5 caps hold for every agent), and sponsor capital is additive, tracked separately as `patronCapitalUsd`. New code: `patrons.ts` (deposit detection — scans `Transfer(to=treasury)` logs, excludes every swarm wallet so cull-unwinds aren't misread, dedupes by txHash, bounded first-run look-back, `getBlockNumber` cacheTime:0), guarded `enqueuePatronSpawns` in `runEpochCycle` (a patron-path failure can never break the core loop), patron-aware funding cap, `/sponsor` page + `SponsorPanel`.
- **Verified by**: `npm run test:fork:patron` — full end-to-end on a Celo mainnet fork: a non-swarm user's deposit detected (and a swarm cull-unwind of equal size correctly excluded), patron agent enqueued in the sponsor's lineage, funded ≤ deposit, ERC-8004 identity registered + self-owned against the forked registry, dedupe + accounting asserted. Then a REAL mainnet deposit as an external user (HD-888 wallet, tx `0xbb31…ee98`, block 69424165) → live swarm spawns the sponsored agent (see below). tsc clean (agent + dashboard); fitness/growth/carryarb suites green.
- **Next**: confirm the live patron spawn, merge to main, demo video.

## Fly handoff + MentoCarryArb live + judge verifier (2026-06-11, afternoon)

- **Done**: Swarm handed off to Fly.io (single writer — the morning's duplicate-instance incident is structurally closed: local stack stopped, machine runs the merged image). MentoCarryArb activated via pendingSpawn injection: **mca-tight, ERC-8004 #9264** — registered, lived epoch 7, culled at −1.71 (held; no cUSD/USDC/USDT spread cleared its 10bps threshold) — the fourth strategy's honest onchain debut and retirement. Growth logic proven in production: epoch 6 spawned a cull replacement AND the first growth spawn (#9265); swarm now mints ~2 identities/epoch at the 1.5h cadence. New `npm run verify:reputation`: recomputes every agent's fitness/score from published inputs and checks them against the onchain giveFeedback calldata (score + feedbackHash) — **epoch 5 verifies 9/9** after completing the report with per-row `epochHours` (crash-resumed settles had reused rows computed under an earlier clock; the hashed payloads always carried the correct value; reports now record it per row).
- **Fly production fixes**: `SIGNAL_URL` was never exported to the swarm process (oracle on :8080, client defaulting to :8402) — every x402 purchase on Fly failed "fetch failed" for ~3h; exported now. `CELO_RPC_URL_BACKUP` secret pointed at an eth-mainnet endpoint — fallback reads went to the wrong chain and blocked spawn retries (ay-chaser-g2-i20); replaced with a Celo RPC.
- **Next**: confirm x402 purchases at the next epoch boundary; ay-chaser-g2-i20 spawn completion; June 13 report polish; June 14 demo video + submission text. Note: local `celo-hackathon` and `spawn-celo/main` (Fly's push target) now diverge by design — merge before the final submission push.
